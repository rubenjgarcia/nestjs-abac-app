...
export class <%= singular(classify(name)) %>Controller {
  ...
  async create(
    ...
    return await this.<%= singular(name) %>Service.create(create<%= singular(classify(name)) %>Dto, request.user);
  }

  @Get()
  @CheckPolicies(new List<%= classify(name) %>PolicyHandler())
  async findAll(@Req() request: Request): Promise<<%= singular(classify(name)) %>[]> {
    return this.<%= singular(name) %>Service.findAll(request.user);
  }
  ...
  async findOne(
    ...
    return this.<%= singular(name) %>Service.findOne(id, request.user);
  }
  ...
  async update(
    ...
    return this.<%= singular(name) %>Service.update(id, update<%= singular(classify(name)) %>Dto, request.user);
  }

  @Delete(':id')
  @CheckPolicies(new Remove<%= singular(classify(name)) %>PolicyHandler('id'))
  remove(@Param('id') id: string, @Req() request: Request) {
    return this.<%= singular(name) %>Service.remove(id, request.user);
  }
}
