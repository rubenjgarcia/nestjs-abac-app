...
describe('UnitService', () => {
  ...
  describe('remove', () => {
    it('should remove a unit', async () => {
      ...
      await unitService.remove(unit._id.toString(), {
        policies: [
          {
            name: 'FooPolicy',
            effect: Effect.Allow,
      ...
      expect((await unitModel.count()).valueOf()).toBe(1); //In every test 2 units are created
    });

    it('should fail to remove a unit if the policies are incorrect', async () => {
      await new unitModel(unit).save();

      await expect(
        unitService.remove(unit._id.toString(), {
          policies: [
            {
              name: 'FooPolicy',
              effect: Effect.Deny,
      ...
      ).rejects.toThrow(`Cannot execute "${RemoveUnit}" on "${UnitScope}"`);
    ...
    it('should remove a unit based on condition', async () => {
      //TODO Must implement this test after create the real schema
    });
  });
});
